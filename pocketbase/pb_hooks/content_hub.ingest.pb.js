...
const { computeTopicKey } = require(__hooks + '/content_hub.lib.dedupe.js');
const { computeSignalScore } = require(__hooks + '/content_hub.lib.scoring.js');

function getRequestInfo(c) {
  try {
...
function getHeader(c, name) {
  ...
  if (headers) {
    ...
    if (headers[target] != null) {
      return Array.isArray(headers[target]) ? headers[target][0] : headers[target];
    }
  }

  try {
    var request = typeof c.request === 'function' ? c.request() : c.request;
...
function appendResurfacedAt(rawPayload, timestamp) {
...
function findFirstRecord(collectionName, filter, params) {
  try {
    return $app.dao().findFirstRecordByFilter(collectionName, filter, params || {});
  } catch (e) {
    return null;
  }
}
...
function createRecord(collectionName, data) {
  var collection = $app.dao().findCollectionByNameOrId(collectionName);
  var record = new Record(collection);
  return saveRecord(record, data || {});
}

function saveRecord(record, data) {
  for (var key in data) {
...
function isOlderThanDays(dateValue, nowMs, days) {
...
function insertSourceRecord(trendId, sourceInput) {
...
function recomputeTrendAggregates(trendId) {
  var trendRecord = $app.dao().findRecordById('ch_trends', trendId);
  var sourceRecords = $app.dao().findRecordsByFilter(
    'ch_trend_sources',
    'trend = {:trendId}',
    '',
    0,
...
routerAdd('POST', '/api/ch/ingest', function (c) {
  ...
  var providedToken = parseBearerToken(getHeader(c, 'Authorization'));
  ...
  var runMetadata = payload.run_metadata || {};
  ...
  try {
    ...
    return c.json(500, { error: 'failed to create ingestion run' });
  }
  ...
  try {
    for (var i = 0; i < trends.length; i++) {
      ...
      try {
        ...
        if (trendRecord) {
          var existing = trendRecord.export();
          var mergedRawPayload = mergeObjects(existing.raw_payload, trend.raw_payload);
          var isResurfaced = isOlderThanDays(existing.last_seen_at, nowMs, 14);
          ...
          trendRecord = saveRecord(trendRecord, {
            topic_key: topicKey,
            topic: topic,
            category: hasOwn(trend, 'category') ? (trend.category || null) : existing.category,
            status: isResurfaced ? 'active' : (existing.status || 'active'),
            first_seen_at: existing.first_seen_at,
            last_seen_at: runStartedAt,
            signal_count: safeInteger(existing.signal_count, 0) + (shouldIncrementSignalCount ? 1 : 0),
            velocity: Math.max(safeNumber(existing.velocity, 0), safeNumber(trend.velocity, 0)),
            views: Math.max(safeNumber(existing.views, 0), safeNumber(trend.views, 0)),
            suggested_title: hasOwn(trend, 'suggested_title') ? normalizeString(trend.suggested_title) : normalizeString(existing.suggested_title),
            angle: hasOwn(trend, 'angle') ? normalizeString(trend.angle) : normalizeString(existing.angle),
            seo_short_tail: hasOwn(trend, 'seo_short_tail') ? normalizeStringArray(trend.seo_short_tail) : normalizeStringArray(existing.seo_short_tail),
            seo_long_tail: hasOwn(trend, 'seo_long_tail') ? normalizeStringArray(trend.seo_long_tail) : normalizeStringArray(existing.seo_long_tail),
            geo_questions: hasOwn(trend, 'geo_questions') ? normalizeStringArray(trend.geo_questions) : normalizeStringArray(existing.geo_questions),
          ...
          trendsNew += 1;
          createdTopicKeysThisRun[topicKey] = true;
