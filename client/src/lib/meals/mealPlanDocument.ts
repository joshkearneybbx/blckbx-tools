...
function buildBlckbxData(input: MealPlanDocumentInput, flat: FlatMeal[]) {
  ...
  return {
    ...
    footerCta: {
      ...
      footline: `Let ${assistantName} know how you'd like to proceed.`,
    },
  };
}
...
export function renderMealPlanDocument(input: MealPlanDocumentInput): string {
  ...
  const assistantName = input.assistantName.trim() || "BlckBx";
  ...
  const blckbxData = buildBlckbxData({ plan, clientName, assistantName }, flat);
...
<section class="hero">
  <div class="wrap">
    ...
    <p class="hero-sub"><strong>Prepared for ${escapeHtml(clientName)}</strong> · compiled by ${escapeHtml(assistantName)}</p>
...
${renderRecipeArticles(flat, plan)}
    ...
    <div class="footer-cta">
      ...
      <p class="cta-footline">Let ${escapeHtml(assistantName)} know how you'd like to proceed.</p>
    </div>
  </div>
</main>
