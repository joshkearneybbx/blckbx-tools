...
export const ICON_PATHS: Record<SocialPlatform, string> = {
...
export function resolvePlatform(key: string): SocialPlatform | null {
...
export function SocialLinksRow({ socialLinks, iconSize = 24, gap = 12, className = '' }: SocialLinksRowProps) {
  ...
  const entries = Object.entries(socialLinks as Record<string, unknown>)
    ...
    .filter((entry): entry is { platform: SocialPlatform; url: string } => entry.platform !== null && typeof entry.url === 'string' && entry.url.trim() !== '');
  ...
  return (
    <div className={`flex items-center ${className}`} style={{ gap }}>
      {entries.map(({ platform, url }) => (
        <a key={platform} href={url} target="_blank" rel="noopener noreferrer" className="text-[hsl(var(--base-black))] transition-colors hover:text-[#6B6865]" aria-label={`${platform} link`}>
